...
| `menu` | `introdus launch` builds the real tmux session (main-control / dev-container, plus the detached notify-host service — no window); the control menu renders live status + grouped sections; Refresh shows the container running; open a dev terminal (`uid=1000(dev)`) and a root terminal (`uid=0(root)`) in the container; copy a host file into `/home/dev/uploads`; add a host to the egress allowlist (persisted to `.introdus/config.env`); Stop → stopped and Restart → running. |
