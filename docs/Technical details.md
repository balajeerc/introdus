...
- A local base image (`introdus-base:latest`) built once from the
  ...
  `ca-certificates`), `mise`, and Node LTS — nothing in the install chain
  needs to run at container start. Coding agents (Claude, Codex, …) are **not**
  baked in; the ones you pick in the wizard install at container setup. Each
  project's container is created from a cheap per-project *tag* of this image
  (`introdus-<project>-<suffix>:latest`, a `podman tag` alias — no rebuild), and
  the container itself is named `introdus-<project>-<suffix>`.
  ...
  the `introdus` wizard and persisted in `.env`, so it's stable across
  ...
  container names, and VS Code state distinct across hosts. `.env` files
  ...
  suffix or a `rebuild-base`) are untagged automatically and a legacy
...
- The repo cloned inside the resulting container, on a per-project
  ...
  on first mount, so mise/node/pnpm come along for free; the agents you picked
  install into `/home/dev` at container setup and then persist on the volume.
...
`introdus-vol-$PROJECT_NAME`, so it survives even a full container
removal (see `introdus reset` below). The rest of the filesystem lives on the
...
container-create time. `podman start` does not re-apply them. So changing the
run configuration — adding a cap, bumping `MEM_LIMIT` via `.env`, exposing a
new port — does not affect an already-created container. To pick up
those changes, remove the container:

```bash
introdus recreate                         # removes container, keeps /home/dev volume
# or
podman rm -f introdus-<project>-<suffix>  # same; see `podman ps` for the exact name
# or
introdus reset                            # removes container AND wipes /home/dev
```
...
Before wiping, `introdus reset` scans `/home/dev/work` in a throwaway container and
...
`introdus reset`. This is an intentional relaxation: the real security boundary
...
`introdus update` runs an in-container refresh against a
...
- `install-agents` — installs any newly-selected agents (idempotent), then
  updates each installed agent in place honouring its install method
  (`pnpm update -g --ignore-scripts`, or `--allow-build` for claude)
- `nvim --headless "+Lazy! sync" +qa` — LazyVim plugin updates

Run it from the control menu ("update"), or from a second host terminal
while the container is up:

```bash
introdus                       # terminal 1: launches the session + control TUI
introdus update                # terminal 2: refreshes in-container
```
...
- It doesn't update image-level non-apt content: the nvim binary at
  ...
  image build. These drift until `introdus rebuild-base` + `introdus reset`.
- It won't recover a broken state. If an `apt upgrade` half-completes,
  it's still your container.

In practice `introdus update` handles the security-patch / day-to-day-freshness
case; `introdus rebuild-base` + `introdus reset` is the escape hatch for
anything structural.

## Sharing host data (read-only)

Set `SHARED_DATA_PATH` in `.env` to an absolute host directory and
`introdus` mounts it read-only at `/home/dev/shared_data` inside the
...
- [sample.env](../sample.env) — config template; copy to `.env` (or let the
  `introdus` wizard write it).
- [Dockerfile](../Dockerfile) — base image. Holds everything shared across
  projects: apt packages, mise, Node LTS. Built once by `introdus`, reused for
  every project, rebuilt with `introdus rebuild-base`.
- **`introdus`** — the single Rust control-plane binary that runs **on the
  container host** (source in [crates/](../crates/)). It replaces the old host
  scripts: validates `.env`, builds the base image if missing, assembles the
  proxy hostname allowlist (git host + `api.trycloudflare.com` added to
  `WHITELIST_HOSTS`), and runs the rootless `--network=pasta` container with
  `--cap-add=NET_ADMIN` so its entrypoint can install the in-container egress
  filter. It **embeds** the container-side bash core below (via `include_str!`)
  and bind-mounts it at launch, so edits to those files apply on a plain
  relaunch with no image rebuild. The host itself does no firewall work and
  needs no `sudo`. Subcommands: `introdus [launch]` (default), `init`, `up`,
  `menu`, `verify`, `recreate`, `reset`, `update`, `rebuild-base`,
  `notify-host`, `notify-listen`, `install`.
...
- [container/agents.sh](../container/agents.sh) + `container/bin/install-agents`
  — the in-container agent registry and installer. `introdus` keeps a
  hand-synced Rust mirror of the registry in
  [crates/introdus-core/src/agents.rs](../crates/introdus-core/src/agents.rs)
  (the wizard checklist + per-agent egress hosts).

The notification path (folded into the `introdus` binary — no Python, no
per-machine installer scripts):

- [container/bin/rc-notify](../container/bin/rc-notify) — in the **dev
  container**. An agent's Stop/Notification hooks call it; it writes
  `event<TAB>project` to the host endpoint mounted at `/run/notify`.
- `introdus notify-host` — on the **container host**, started automatically as a
  detached service for each launched tmux session. Reads the endpoint, validates
  the event against a fixed whitelist and sanitizes the label, then either
