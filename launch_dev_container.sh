...
if ! $VERIFY; then
    if [[ $OS == "macos" ]]; then
        ...
        # current transport.
        NOTIFY_UNIT_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/systemd/user/rc-notify.service"
        if systemctl --user is-active --quiet rc-notify.service 2>/dev/null && notify_exists; then
            echo "==> rc-notify listener already running"
        elif [[ -f "$NOTIFY_UNIT_FILE" ]]; then
            # host_install.sh installed a persistent unit; just (re)start it. We
            # must NOT use systemd-run here — its transient unit would collide
            # with the installed unit of the same name.
            ...
            # No installed unit (run-in-place without host_install.sh): fall back
            # to a transient unit so notifications still work.
            echo "==> starting rc-notify listener on $NOTIFY_SOCK"
