...
| TA125 | Paseo can launch only its native providers: `agents::paseo::supports` is true for claude/codex/opencode/pi, false for antigravity/kilocode; every listed provider is a known agent id | ✅ unit `introdus-core` | 1 | ta125_paseo_supports_only_native_providers |
...
| TA128 | Paseo end to end via the TUI: "Install paseo" installs `@getpaseo/cli` (INSTALL_PASEO + paseo.sh persisted to `.env`), "Launch an installed agent" offers a "via paseo" mode for claude that spawns a `paseo-claude` window, and "Show paseo pairing QR code" spawns a `paseo-qr` window. Both windows run paseo as a single quoted `podman exec` **inside the container** (asserted via a live `podman exec $cname …` carrying the script's `exec …` tail), not leaked to the host shell. (A physically paired phone is out of scope — no device; daemon↔relay connectivity is asserted separately in TA130.) | ✅ harness `paseo` | 1 | driver-paseo.sh: opt-in via `.env` → paseo auto-installed on launch → Launch → "via paseo" offer → `paseo-claude` window + in-container assert → QR → `paseo-qr` window + in-container assert |
