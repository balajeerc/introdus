...
| TA142 | "Quit this tmux session (leave the container running)" tears down the whole tmux session (every window closed) but leaves the container running (reattachable on the next launch), unlike TA124 which also stops it | ✅ harness `quit-session` | 1 | driver-quit-session.sh: pick it (no confirm) → `tmux has-session` fails + container still `State.Running=true` |
